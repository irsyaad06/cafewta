...
const form = useForm({
    cafe_table_id: props.cafeTable?.id || '',
    payment_method_id: '',
    cart: []
});
...
// No changeAmount calculation needed for Customer
...
const submitCheckout = () => {
    ...
    form.post(route('order.store'), {
        ...
        onSuccess: () => {
            cart.value = [];
            closeCheckout();
            alert('Pesanan berhasil dibuat! Silakan tunggu atau bayar di kasir.');
        }
    });
};
    ...
    <div class="flex flex-col md:flex-row h-screen bg-gray-50 overflow-hidden font-sans pb-16 md:pb-0">
        ...
        >
            ...
            <div class="p-4 md:p-6 border-b border-gray-200">
                <h1 class="text-xl md:text-2xl font-bold text-gray-800 mb-4">Pesan Menu (Meja {{ cafeTable?.table_number }})</h1>
        ...
        <div v-if="isCheckoutModalOpen" class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden transform transition-all max-h-[95vh] flex flex-col">
                ...
                <form @submit.prevent="submitCheckout" class="p-6 sm:p-8 overflow-y-auto">
                    ...
                    <div class="space-y-5">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Meja</label>
                            <div class="w-full rounded-xl border border-gray-300 bg-gray-100 py-3 px-4 font-medium text-gray-800">
                                Meja {{ cafeTable?.table_number }} - {{ cafeTable?.name }}
                            </div>
                        </div>
                        ...
                        <!-- Sembunyikan bagian Uang Diterima & Kembalian untuk Self Order -->
                    </div>
                    ...
                    <div class="mt-8 flex gap-3">
                        ...
                        <button 
                            type="submit" 
                            :disabled="form.processing || !form.payment_method_id"
                            class="w-2/3 py-3 rounded-xl font-bold text-white shadow-lg transition-all"
                            :class="(form.processing || !form.payment_method_id) ? 'bg-gray-400 cursor-not-allowed shadow-none' : 'bg-primary-600 hover:bg-primary-700 hover:shadow-primary-500/30'"
                        >
